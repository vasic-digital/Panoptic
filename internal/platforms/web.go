...
func (w *WebPlatform) StartRecording(filename string) error {
	...
	// Use CDP screencast recorder for real video capture
	log := *logger.NewLogger(false)
	w.recorder = NewScreencastRecorder(w.page, log)

	if err := w.recorder.Start(filename); err != nil {
		w.recording = false
		return fmt.Errorf("failed to start screencast recording: %w", err)
	}
...
func (w *WebPlatform) StopRecording() error {
	...
	// Stop the screencast recorder
	if w.recorder != nil {
		if err := w.recorder.Stop(); err != nil {
			w.metrics["recording_error"] = err.Error()
			return fmt.Errorf("failed to stop screencast recording: %w", err)
		}

		// Update metrics with real recording info
		w.metrics["video_saved"] = true
		w.metrics["video_frames"] = w.recorder.FrameCount()

		if info, err := os.Stat(w.recorder.filename); err == nil {
			w.metrics["video_size"] = info.Size()
		}
